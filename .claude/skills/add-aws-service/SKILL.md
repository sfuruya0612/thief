...
対応する AWS 公式 Architecture Icons(32px)のファイル名を `frontend/scripts/fetch-aws-icons.mjs` の `ICON_FILENAMES` にも追加する(このファイル名は AWS 公式アイコンパッケージ zip 内のファイル名と一致させること)。SVG 本体はライセンス上リポジトリにコミットしない(`.gitignore` 対象、`mise run frontend:fetch-aws-icons <zip>` で取得)。該当する Architecture Icon が存在しない場合のみ Resource Icons を代替として使う(natgw の実例を参照)。
