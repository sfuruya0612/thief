...
- [UPDATE] Datadog Metrics のクエリ入力・実行済みクエリ・期間の状態を `DatadogMetricsView` から `DatadogView` へリフトアップし、Cost/Dashboards セクションへ切り替えてから Metrics に戻っても入力が失われないようにする (`DatadogMetricsView` はセクション切り替えで条件レンダリングによりアンマウントされ、ローカル state を持っていたため従来は戻るたびに空欄と既定期間に戻っていた。組織タブを切り替えたときにクエリが持ち越されない挙動は `DatadogView` を再マウントする既存の `key={datadogOrg}` により従来どおり変わらない)
  - @sfuruya0612
- [UPDATE] EC2 の Running instances と ECS の Tasks per cluster のグラフで、開いた直後に表示する期間の既定を 1 day から 7 days に変える (期間ボタンで 1 day と 1 month に切り替えられることは変わらない。EC2 の台数は一覧の取得時にだけ記録されるため、1 日の窓では点が少なく推移が読めなかった)
  - @sfuruya0612
...
- [ADD] EC2 の Running インスタンス数と ECS のクラスターごとのタスク数の推移を折れ線グラフで表示できるようにする (期間は 1 日 / 7 日 / 1 か月を切り替えられ、粒度はそれぞれ 60 秒 / 300 秒 / 3600 秒。ECS は新規導入した `github.com/aws/aws-sdk-go-v2/service/cloudwatch` で `AWS/ECS` `LiveTaskCount` をメトリクス演算 `SUM(SEARCH(...))` によりクラスター単位に合算して取得する (`cloudwatch:GetMetricData` が無い場合は 403 で理由を表示しグラフを出さない)。EC2 には account/region 単位の集計メトリクスが無いため、`handleEC2` が AWS から一覧を実際に取得したとき (キャッシュ MISS と Refresh) だけ Running 台数を profile と region ごとのプロセス内固定長リングバッファ (上限 4096 点) に記録し、時系列エンドポイントはこれを切り出して返す。時系列の取得は一覧の取得とは別の TanStack Query で行い、一覧の表示を待たせない)
  - @sfuruya0612
- [ADD] EC2 の台数の推移グラフを、Auto Scaling グループごとの InService インスタンス数 (CloudWatch `AWS/AutoScaling` の `GroupInServiceInstances`) に変更する (AWS/EC2 名前空間にはアカウントやリージョン単位の Running 台数を表す標準メトリクスが無いため、CloudWatch の `GetMetricData` に時間窓を指定してグループごとに 1 系列を取得する。`backend/internal/aws/autoscaling.go` の `ListAutoScalingGroupNames` が `DescribeAutoScalingGroups` でグループ名を列挙し、`backend/internal/aws/ec2_metrics.go` の `ListEC2InstanceCountSeries` が AWS/AutoScaling の `GroupInServiceInstances` を MetricStat で引いて欠測を null 埋めする。応答は他の時系列と同じく `serveCached` でキャッシュし、期間をキャッシュキーに含める。Auto Scaling グループに属さないインスタンスは含まれないため、グラフの右下にその旨の注記を出す。これに伴い、プロセス内で記録していた `EC2CountRecorder` と定期サンプラー、`handleEC2` の記録を削除する。`autoscaling:DescribeAutoScalingGroups` の権限が新たに必要になる)
  - @sfuruya0612
